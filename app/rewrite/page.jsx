...
const STEPS = ['uploading', 'analyzing', 'rewriting'];
...
function stepState(currentStep, badgeStep) {
  const order = ['uploading', 'analyzing', 'rewriting', 'done'];
  ...
  if (compileStep === 'compiling') {
    return (
      <div className="w-full h-full bg-gray-900 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-pink-400 font-black uppercase text-sm">Compilation du LaTeX...</p>
        <p className="text-gray-500 font-mono text-xs">Quelques secondes...</p>
      </div>
    );
  }
    ...
    return (
      <div className="w-full h-full bg-gray-900 flex flex-col items-center justify-center gap-4 p-8">
        <div className="bg-pink-400 border-4 border-black p-6 shadow-[4px_4px_0px_0px_#000] max-w-md w-full">
          <p className="font-black uppercase text-black text-sm mb-2">Compilation échouée</p>
          <p className="font-bold text-black text-xs">
            Le source généré contient peut-être des paquets non supportés.
          </p>
        </div>
        <button
          onClick={onRetry}
...
export default function RewritePage() {
  ...
  const handleRewrite = async () => {
    ...
    try {
      const response = await fetch('/api/rewrite', { method: 'POST', body: formData });
      if (!response.ok) {
      ...
      setErrorMsg(err.message || 'Rewrite failed');
      setStep('error');
    }
  };
  ...
  const isLoading = step === 'uploading' || step === 'analyzing' || step === 'rewriting';
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 px-5 py-3 border-b-4 border-black bg-white flex items-center gap-3 flex-wrap">
          ...
          <>
            ...
            <div className="flex items-center gap-2 ml-auto shrink-0">
              <div className="w-32 bg-gray-200 border-2 border-black h-3 hidden sm:block">
                <div className="h-full bg-pink-400 transition-all duration-300" style={{ width: `${progress}%` }} />
              </div>
          ...
          <>
            ...
            <div className="flex gap-2 ml-auto flex-wrap">
              ...
              <a
                ...
                download={`${file?.name?.replace(/\.pdf$/i, '') ?? 'document'}-rewrite.tex`}
        ...
        {step === 'idle' && (
          <div className="flex gap-3 ml-auto">
            ...
            <Link href="/convert" className="font-black py-1.5 px-3 bg-cyan-400 border-2 border-black shadow-[2px_2px_0px_0px_#000] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px] transition-all uppercase text-xs">
              CONVERT
            </Link>
          </div>
        )}
          ...
          <div className="max-w-[700px] mx-auto space-y-6">
            <div
              ...
              className={`border-8 border-black border-dashed p-16 text-center cursor-pointer transition-all shadow-[8px_8px_0px_0px_#000] ${
                dragOver ? 'bg-pink-400' : 'bg-white hover:bg-pink-50'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              {file ? (
                <>
                  ...
                  <p className="text-base font-bold text-gray-600 mt-2">
                    {(file.size / 1024).toFixed(0)} KB — Cliquer pour changer
                  </p>
                </>
              ) : (
                <>
                  <p className="text-2xl font-black uppercase text-black">Déposer un PDF ici</p>
                  <p className="text-base font-bold text-gray-600 mt-2">ou cliquer pour parcourir</p>
                </>
              )}
            </div>
            ...
            <button
              ...
              className={`w-full py-5 text-2xl font-black uppercase border-4 border-black shadow-[6px_6px_0px_0px_#000] transition-all ${
                file
                  ? 'bg-pink-400 hover:shadow-[3px_3px_0px_0px_#000] hover:translate-x-[3px] hover:translate-y-[3px] cursor-pointer'
      ...
      {/* Loading: split (original PDF | streaming LaTeX) */}
      {isLoading && (
        <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2">
          ...
          <div
            ref={codeRef}
            className="bg-gray-900 overflow-y-auto p-4 font-mono text-sm text-pink-400 whitespace-pre-wrap"
          >
            {latexCode}
      ...
      {/* Done: split (LaTeX source | compiled PDF) */}
      {step === 'done' && (
        <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2">
          <div className="hidden md:block border-r-4 border-black bg-gray-900 overflow-y-auto p-4 font-mono text-sm text-pink-400 whitespace-pre-wrap">
            {latexCode}
          </div>
      ...
      {step === 'error' && (
        <div className="flex-1 overflow-y-auto flex items-center justify-center px-5">
          <div className="bg-red-400 border-4 border-black p-8 shadow-[6px_6px_0px_0px_#000] max-w-lg w-full">
            <p className="font-black uppercase text-black text-lg mb-2">Erreur de réécriture</p>
